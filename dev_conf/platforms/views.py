...
from platforms.forms import PlatformForm, DeviceForm, PlatformTransferForm
...
def transfer_devices(request):
    context = RequestContext(request)
    devices_list = Device.objects.order_by('-owner')
    platforms = Platform.objects.order_by('-name')
    context_dict = {'devices': devices_list, 'platforms': platforms}

    if request.method == 'POST':
        device_to_transfer = request.POST.getlist('ip_list')
        print device_to_transfer
        platform = request.POST.getlist('platform')
        print platform

    return render_to_response('platforms/transfer_devices.html', context_dict, context)